...
# Embed conference rank into the corresponding metric
SDSU_stats <- paste0(SDSUpergamestats_filtered["SDSU", ], " (", SDSUpergamestats_filtered["SDSU Conference Rank", ], ")")
Opponent_stats <- paste0(SDSUpergamestats_filtered["SDSU Opponent", ], " (", SDSUpergamestats_filtered["SDSU Opponent Rank", ], ")")

# Create a condensed data frame
condensed_table <- data.frame(
  Metric = colnames(SDSUpergamestats_filtered),
  `SDSU (Rank)` = SDSU_stats,
  `Opponent (Rank)` = Opponent_stats
)
...
pander(SDSUpergamestats_filtered, caption = "<span style='font-size: 20px; font-weight: bold;'>SDSU vs. Opponent per game stats with Conference Rank</span>")
...
# Mutations
sdsu_shots2 <- sdsu_shots2 %>% mutate(temp = x, x = y, y = temp) %>% select(-temp)
...
p1 = plot_court(court_themes$ppt, use_short_three = T) +
  geom_point(data = sdsu_shots2, aes(x = x, y = y, color = outcome, fill = outcome), 
...
# SDSU vs. UNM, UAB, Yale, and UConn
...
sdsu_shots4 <- sdsu_shots4 %>% mutate(y = 46-y)  
#sdsu_shots4 = sdsu_shots4 %>% mutate(y= y*-1)
...
p1 = plot_court(court_themes$ppt, use_short_three = T) +
  ...
  theme(plot.title = element_text(hjust = .5, size = 22, family = "Comic Sans MS", face = "bold", vjust = -4),
        ...
        legend.text = element_text(hjust = .5, size = 10, family = "Comic Sans MS", face = "bold", colour = "white"),
        plot.caption = element_text(hjust = .5, size = 6, family = "Comic Sans MS", face = "bold", colour = "lightgrey", vjust = 8)) +
  ggtitle(label = "SDSU Aztecs vs. UNM, UAB, Yale, and UConn",
          subtitle = "Season Record 26-11") 
...
correlations <- c(Points = 0.5734, FG_percent = 0.4563, 
                  X3P_percent = 0.391, FTA = 0.345, 
...
pander(starting_lineup[, c("Player", "Position", "Score")], row.names = FALSE, caption = "<span style='font-size: 20px; font-weight: bold;'>Best Starting Lineup</span>")
```